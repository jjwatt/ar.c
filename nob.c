...
bool build_ar_lib(Nob_Cmd *cmd) {
    const char *bin_path = BUILD_FOLDER"ar.o";
    const char *src_path = SRC_FOLDER"ar.c";
    ...
    /* Create static lib */
    nob_cmd_append(cmd, "ar", "rcs", BUILD_FOLDER"libar.a", bin_path);
    if (!nob_cmd_run_sync_and_reset(cmd)) return false;
    nob_da_free(source_paths);
    return true;
}
